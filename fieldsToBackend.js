...
const fieldsToBackend = (address) => {
  ...
  if (address.premise) {
    ...
    lines.push({seq: 0, type: {value: 'Premise'}, value: address.premise, id})
  } else {
  ...
  if (address.thoroughfare) {
    ...
    lines.push({seq: 1, type: {value: 'Thoroughfare'}, value: address.thoroughfare, id})
  }
  if (address.postalCodeOrTown) {
    const id = getExistingLineField(address.lines, 'postalcodeortown')?.id;
    lines.push({seq: 2, type: {value: 'PostalCodeOrTown'}, value: address.postalCodeOrTown, id})
  } else {
    lines.push(deleteFieldIfExists(address.lines, 'postalcodeortown'));
  }
  if (address.locality) {
    const id = getExistingLineField(address.lines, 'locality')?.id;
    lines.push({seq: 3, type: {value: 'Locality'}, value: address.locality, id})
  }
  if (address.administrativeArea) {
    const id = getExistingLineField(address.lines, 'administrativearea')?.id;
    lines.push({seq: 4, type: {value: 'AdministrativeArea'}, value: address.administrativeArea, id})
  }
  if (address.postalCode) {
    const id = getExistingLineField(address.lines, 'postalcode')?.id;
    lines.push({seq: 5, type: {value: 'PostalCode'}, value: address.postalCode, id})
  }
  if (address.country) {
    const id = getExistingLineField(address.lines, 'country')?.id;
    lines.push({seq: 6, type: {value: 'Country'}, value: address.country, id})
  }
